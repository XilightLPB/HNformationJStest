...
function TestTheGrid(table){
    ...
    for (let i = 0; i<9; i++){
        let isGood = TestRow(table[i]);

        //creating an issue containing at the first index the error message, and then each element of the problematic row
        if(!isGood){
            console.log(`There was an issue with the row ${i}, values of the problematic row:`);
            console.log(table[row]);
            let issue =[];
            for (let j = 0; j<10; j++){
                if (i==0){
                    issue.push(`There was an issue with the row ${i}, values of the problematic row:`);
                }
                else{
                    issue.push(table[i][j-1]);
                }
            }
            issues.push(issue);
